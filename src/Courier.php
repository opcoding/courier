<?php
        ...
        class Courier implements Plugin
        {
            ...
            public function runHook($name, $host, $remotePath)
            {
                ...
                if (file_exists($hookFullPath))
                {
                    ...
                    if ($result)
                    {
                        $this->phpci->log(sprintf('Ran "%s" hook on "%s"', $name, $host));
                    }
                    else
                    {
    ...
    // alias for better integration
    //
    // this class allows to refer the plugin as "courier" in phpci.yml

    namespace PHPCI\Plugin {

        class Courier extends \OpCoding\Courier\Courier
        {
